...
var searchEntry = document.getElementById('form-entry');
var historyEl = document.getElementById('location-hist');
...
function getCityFips(locationFullName) {
  ...
  var plainFullName = locationFullName.normalize("NFD").replace(/[\u0300-\u036f]/g, ""); // Convert Address to City or County FIPS ID

  return fetch('https://datausa.io/api/searchLegacy/?q=' + plainFullName).then(function (response) {
...
function GetLocationInfo(location) {
  fetch('https://api.opencagedata.com/geocode/v1/json?q=' + location + '&key=c740ebf9cdd34742986f08e8fd78033c&language=en&pretty=1&countrycode=us').then(function (response) {
    return response.json();
  }).then(function (data) {
    // Location Lattitude and Longitude
    var lat = data.results[0].geometry.lat;
    var lng = data.results[0].geometry.lng;
    searchAQIResult(lat, lng); // Location Name Information
    ...
    saveHistory(fullName); // Call APIs Via Separate Function Calls 
